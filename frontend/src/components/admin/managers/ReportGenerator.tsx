...
export default function ReportGenerator({ users, transactions }: ReportGeneratorProps) {
  ...
  const userMap = useMemo(() => new Map(users.map(user => [user.id_user, user])), [users]);
    ...
    switch (reportType) {
      ...
      case 'bottle_transactions':
        data = transactions
          ...
          .map(tx => ({
            ...tx,
            userName: userMap.get(tx.id_user)?.name || 'N/A',
            petugasName: userMap.get(tx.id_petugas)?.name || 'N/A',
          }));
        break;
      ...
      case 'officer_activity':
        data = transactions
          .filter(tx =>
            ...
            (officerFilter === 'all' || tx.id_petugas === Number(officerFilter))
          )
          .map(tx => ({
            ...tx,
            userName: userMap.get(tx.id_user)?.name || 'N/A',
      ...
      case 'user_ranking':
        ...
        users.forEach(u => {
          userStats.set(u.id_user, { name: u.name, nik: u.nik || '-', points: u.points, bottles: 0 });
        });

        transactions.forEach(tx => {
          if (tx.type === 'bottle_exchange' && tx.status === 'completed' && isInDateRange(tx.created_at)) {
            const stat = userStats.get(tx.id_user);
            if (stat) {
                ...
                <Select value={officerFilter} onValueChange={setOfficerFilter}>
                  ...
                  <SelectContent>
                    ...
                    {officers.map(officer => (
                      <SelectItem key={officer.id_user} value={String(officer.id_user)}>
                        {officer.name}
                      </SelectItem>
                    ))}
                  </SelectContent>
